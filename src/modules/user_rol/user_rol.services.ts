...
export class UserRolService {
  ...
  async create(userRol_dto: CreateUserRolDto): Promise<UserRol> {
    ...
    const rols: Rol = await this.rolRepository.findOneBy({
      id: userRol_dto.rol_id,
    });
    const users: User = await this.userRepository.findOneBy({
      id: userRol_dto.user_id,
    });
  ...
  async updateUserRolById(
    id: number,
    updateData: Partial<UpdateUserRolDto>,
  ): Promise<UserRol> {
    const userRol = await this.userRolRepository.findOneBy({ id: id });
    if (!userRol) {
      throw new NotFoundException('user rol not found');
