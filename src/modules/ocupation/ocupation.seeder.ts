...
import { CreateOcupationDto } from "src/controllers/ocupation/ocupation.dto";
...
export class OcupationSeeder implements OnModuleInit {
    ...
    async onModuleInit() {
        ...
        if (exist.length > 0) {
            console.log('Saltando proceso del seeder -> (Ocupation). Ya existen registros en la base de datos.');
            return;
        }
        console.log('Iniciando el seeder -> (Ocupation). Cargando registros en la base de datos.');
