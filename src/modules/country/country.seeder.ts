...
export class CountrySeeder implements OnModuleInit {
    ...
    async onModuleInit() {
        ...
        if (exist.length > 0) {
            console.log('Saltando proceso del seeder -> (Country). Ya existen registros en la base de datos.');
            return;
        }
        console.log('Iniciando el seeder -> (Country). Cargando registros en la base de datos.');
        const countriesData = [
