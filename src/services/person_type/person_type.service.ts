...
import { CreatePersonTypeDto, UpdatePersonTypeDto } from 'src/controllers/person_type/person_type.dto';
...
export class PersonTypeService {
    ...
    async updateById(id: number, updateData: UpdatePersonTypeDto): Promise<PersonType> {
    ...
    async updatePersonTypeById(id: number, updateData: Partial<UpdatePersonTypeDto>): Promise<PersonType> {
        const personType = await this.personTypeRepository.findOneBy({id:id});
        if (!personType) {
            throw new NotFoundException('Person Type not found');
        }
