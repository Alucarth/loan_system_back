...
export class BranchController {
    ...
    async create(@Body() branchData: CreateBranchDto){
        ...
        let response = await this._branchService.create(branchData)
        return response
    }
    @Get(':id')
    @HttpCode(HttpStatus.OK)
    ...
    @Put(':id')
    @HttpCode(HttpStatus.OK)
    async updateById(@Param('id', ParseIntPipe) id: number, @Body() updateData: UpdateBranchDto) {
        return this._branchService.updateById(id, updateData);
    }

    @Patch(':id')
    @HttpCode(HttpStatus.OK)
    async updateAccountById(@Param('id', ParseIntPipe) id: number, @Body() updateData: Partial<UpdateBranchDto>) {
