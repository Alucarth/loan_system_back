...
export class UserRolController {
    ...
    async create(@Body() userRolData: CreateUserRolDto){
        ...
        let response = await this._userRolService.create(userRolData)
    ...
    @Put(':id')
    @HttpCode(HttpStatus.OK)
    async updateById(@Param('id', ParseIntPipe) id: number, @Body() updateData: UpdateUserRolDto) {
        return this._userRolService.updateById(id, updateData);
    }

    @Patch(':id')
    @HttpCode(HttpStatus.OK)
    async updateUserRolById(@Param('id', ParseIntPipe) id: number, @Body() updateData: Partial<UpdateUserRolDto>) {
