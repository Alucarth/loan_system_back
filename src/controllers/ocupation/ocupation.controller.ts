...
export class OcupationController {
    ...
    async create(@Body() ocupationData: CreateOcupationDto){
        ...
        let response = await this._ocupationService.create(ocupationData)
        return response
    }
    
    @Put(':id')
    @HttpCode(HttpStatus.OK)
    async updateById(@Param('id', ParseIntPipe) id: number, @Body() updateData: UpdateOcupationDto) {
        return this._ocupationService.updateById(id, updateData);
    }

    @Patch(':id')
    @HttpCode(HttpStatus.OK)
    async updateOcupationById(@Param('id', ParseIntPipe) id: number, @Body() updateData: Partial<UpdateOcupationDto>) {
